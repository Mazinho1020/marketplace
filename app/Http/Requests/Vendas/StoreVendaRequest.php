...
class StoreVendaRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'tipo_venda' => 'required|in:' . implode(',', array_keys(Venda::TIPOS_VENDA)),
            'origem_venda' => 'required|in:' . implode(',', array_keys(Venda::ORIGENS_VENDA)),
    ...
    public function messages(): array
    {
        return [
            ...
            'origem_venda.required' => 'A origem da venda é obrigatória.',
            ...
            'itens.min' => 'É necessário pelo menos um item na venda.',
    ...
    protected function validateCustomRules($validator)
    {
        ...
        if ($this->cliente_id && $this->empresa_id) {
        ...
        if ($this->vendedor_id && $this->empresa_id) {
            $vendedor = \App\Models\User::whereHas('empresas', function ($query) {
            ...
            foreach ($this->itens as $index => $item) {
                if (isset($item['produto_id'])) {
                    ...
                    if ($produto && $produto->controla_estoque) {
                        if ($produto->estoque_atual < ($item['quantidade'] ?? 0)) {
