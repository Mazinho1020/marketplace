...
class EmpresaUsuario extends Authenticatable
{
    ...
    /**
     * Todas as empresas relacionadas ao usuário (vinculadas)
     * Para compatibilidade com o sistema antigo
     */
    public function empresasProprietario(): BelongsToMany
    {
        return $this->belongsToMany(Empresa::class, 'empresa_user_vinculos', 'user_id', 'empresa_id')
            ->withPivot(['perfil', 'status', 'permissoes', 'data_vinculo'])
            ->withTimestamps()
            ->wherePivot('perfil', 'proprietario');
    }
    ...
    {
        $empresasProprietario = $this->empresasProprietario()->get();
        $empresasVinculadas = $this->empresasVinculadasAtivas()->get();
    ...
    {
        // Verifica vínculo na tabela de relacionamento
        ...
        // Se é proprietário via vínculo, tem todas as permissões
        if ($vinculo->pivot->perfil === 'proprietario') {
            return true;
        }

        // Se não especificou permissão específica, apenas verifica se tem vínculo ativo
        if (!$permissao) {
            return $vinculo->pivot->status === 'ativo';
        }

        // Verifica permissão específica no JSON de permissões
    ...
    public function getEstatisticasDashboard(): array
    {
        ...
        $todasEmpresas = $this->getTodasEmpresasAttribute();
        $empresas = $todasEmpresas->count();
        $empresasAtivas = $todasEmpresas->where('status', 'ativa')->count();
