...
class ContasPagarService
{
    ...
    public function pagar(int $id, float $valor, array $dadosPagamento): bool
    {
        return DB::transaction(function () use ($id, $valor, $dadosPagamento) {
            $lancamento = LancamentoFinanceiro::findOrFail($id);
            
            // Verificar se é conta a pagar
            if (!$lancamento->isContaPagar()) {
                throw new \Exception('Este lançamento não é uma conta a pagar');
            }
            
            // Verificar se já não está pago
            if ($lancamento->isPaga()) {
                throw new \Exception('Esta conta já está paga');
            }
            ...
            // Criar pagamento
            $pagamento = $lancamento->adicionarPagamento(array_merge($dadosPagamento, [
                'valor' => $valor,
                'data_pagamento' => $dadosPagamento['data_pagamento'] ?? now(),
                'status' => 'confirmado',
            ]));
            
            return $pagamento->exists;
        });
    }
    ...
    public function getDashboard(int $empresaId): array
    {
        ...
        return [
            ...
            'total_pago_mes' => $baseQuery->clone()
    ...
    {
        $contasVencidas = LancamentoFinanceiro::where('natureza_financeira', NaturezaFinanceiraEnum::PAGAR)
            ->where('situacao_financeira', SituacaoFinanceiraEnum::PENDENTE)
