...
class CacheService
{
    ...
    public function getMerchantFeatures($merchantId): array
    {
        ...
        return Cache::remember($cacheKey, self::PLAN_TTL, function () use ($merchantId) {
            // Buscar merchant e seu plano através das tabelas reais
            $merchant = DB::table('funforcli')->where('id', $merchantId)->first();

            if (!$merchant) {
                return $this->getDefaultFeatures();
            }

            // Buscar empresa do merchant
            $empresa = DB::table('empresas')->where('id', $merchant->empresa_id ?? $merchantId)->first();

            if (!$empresa) {
                return $this->getDefaultFeatures();
            }

            // Por enquanto, assumir plano básico para todos
            // Pode ser expandido quando houver sistema de assinatura implementado
            $plan = DB::table('afi_plan_planos')->where('codigo', 'basico')->first();
        ...
        return Cache::remember($cacheKey, self::STATS_TTL, function () use ($merchantId) {
            $query = "
                SELECT 
                    ...
                    COALESCE(SUM(CASE WHEN t.status = 'completed' THEN t.final_amount ELSE 0 END), 0) as volume_aprovado,
                    COALESCE(AVG(CASE WHEN t.status = 'completed' THEN t.final_amount END), 0) as ticket_medio
