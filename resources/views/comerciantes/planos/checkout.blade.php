...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-8">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p class="mb-1">{{ $plano->nome ?? 'Plano' }}</p>
                            <small class="text-muted">{{ $plano->descricao ?? '' }}</small>
                        </div>
                        <div class="col-md-3">
                            <h6>Ciclo</h6>
                            <p class="mb-0">{{ ucfirst($transaction->metadados['ciclo_cobranca'] ?? 'mensal') }}</p>
                        </div>
                        <div class="col-md-3">
                            <h6>Valor</h6>
                            <h4 class="text-primary mb-0">
                                R$ {{ number_format($transaction->valor_final, 2, ',', '.') }}
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <!-- Status da Transação -->
            @if($transaction->status === 'pendente' && $transaction->forma_pagamento)
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @if($transaction->forma_pagamento === 'pix')
                    <!-- PIX -->
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <button class="btn btn-success w-100" onclick="confirmarPagamento('{{ $transaction->uuid }}')">
                                <i class="fas fa-check me-2"></i>Já Paguei
                            </button>
                        </div>
                    </div>

                    @elseif($transaction->forma_pagamento === 'bank_slip')
                    <!-- Boleto -->
                    <div class="text-center">
                        ...
                        <div class="alert alert-info">
                            ...
                            <p class="mb-1">• Vencimento: {{ now()->addDays(3)->format('d/m/Y') }}</p>
                        ...
                        <button class="btn btn-success" onclick="confirmarPagamento('{{ $transaction->uuid }}')">
                            <i class="fas fa-check me-2"></i>Já Paguei
                        </button>
                    </div>
            ...
            @elseif($transaction->status === 'aprovado')
            <!-- Pagamento Aprovado -->
            <div class="card">
                ...
                <div class="card-body text-center">
                    ...
                    <p class="mb-4">Seu plano {{ $plano->nome ?? '' }} está ativo e você já pode aproveitar todos os recursos.</p>
            ...
            <!-- Outros Status -->
            <div class="card">
                <div class="card-header bg-{{ $transaction->status_cor }}">
                    <h5 class="mb-0 text-white">
                        <i class="{{ $transaction->status_icone }} me-2"></i>
...
<script>
    function confirmarPagamento(uuid) {
        if (confirm('Você confirma que já realizou o pagamento?')) {
            fetch(`{{ route('comerciantes.planos.confirmar-pagamento', '') }}/${uuid}`, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = data.redirect || '{{ route("comerciantes.planos.dashboard") }}';
                    } else {
    ...
    // Countdown para PIX
    @if($transaction - > expira_em ?? false)
    const expiresAt = new Date('{{ $transaction->expira_em->toISOString() }}');
