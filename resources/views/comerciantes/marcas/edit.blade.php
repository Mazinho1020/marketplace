...
@section('title', 'Editar Marca - ' . $marca->nome)

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-0">Editar Marca</h1>
        <p class="text-muted mb-0">{{ $marca->nome }}</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('comerciantes.marcas.show', $marca) }}" class="btn btn-outline-secondary">
            <i class="fas fa-arrow-left me-1"></i>
            Voltar
        </a>
        <a href="{{ route('comerciantes.marcas.index') }}" class="btn btn-outline-primary">
...
<form method="POST" action="{{ route('comerciantes.marcas.update', $marca) }}" enctype="multipart/form-data" class="needs-validation" novalidate>
    @csrf
    @method('PUT')
        ...
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="ativa" {{ old('status', $marca->status) == 'ativa' ? 'selected' : '' }}>Ativa</option>
                                <option value="inativa" {{ old('status', $marca->status) == 'inativa' ? 'selected' : '' }}>Inativa</option>
                                <option value="suspensa" {{ old('status', $marca->status) == 'suspensa' ? 'selected' : '' }}>Suspensa</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" 
                               id="slug" name="slug" value="{{ old('slug', $marca->slug) }}" 
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('descricao') is-invalid @enderror" 
                                  ...
                                  placeholder="Descreva sua marca, valores, diferenciais...">{{ old('descricao', $marca->descricao) }}</textarea>
                        @error('descricao')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="color" class="form-control form-control-color @error('cor_primaria') is-invalid @enderror" 
                                       ...
                                       value="{{ old('cor_primaria', $marca->identidade_visual['cor_primaria'] ?? '#2ECC71') }}">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="color" class="form-control form-control-color @error('cor_secundaria') is-invalid @enderror" 
                                       ...
                                       value="{{ old('cor_secundaria', $marca->identidade_visual['cor_secundaria'] ?? '#27AE60') }}">
                                <input type="text" class="form-control" 
                                       value="{{ old('cor_secundaria', $marca->identidade_visual['cor_secundaria'] ?? '#27AE60') }}"
                                       readonly>
                            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <a href="{{ route('comerciantes.marcas.show', $marca) }}" class="btn btn-outline-secondary w-100">
                ...
                <div class="card-body">
                    ...
                    @if($marca->logo_url)
                        <div class="text-center">
                            ...
                            <img src="{{ $marca->logo_url }}" alt="Logo atual" 
                ...
                <div class="card-body">
                    <div class="small">
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span>{{ $marca->created_at->format('d/m/Y') }}</span>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">Atualizada em:</span>
                            <span>{{ $marca->updated_at->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
