...
@section('title', 'Editar Configuração de Produto')
...
<style>
    ...
    .existing-item { background-color: #f8f9fa; }
    .new-item { background-color: #e8f5e8; }
</style>
@endsection
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">
                ...
                Editar Configuração: {{ $configuracao->nome }}
            </h1>
        ...
        <div class="btn-group">
            <a href="{{ route('comerciantes.produtos.configuracoes.show', $configuracao) }}" class="btn btn-outline-info">
    ...
    <form action="{{ route('comerciantes.produtos.configuracoes.update', $configuracao) }}" method="POST" id="configuracaoForm">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('nome') is-invalid @enderror" 
                                           id="nome" name="nome" value="{{ old('nome', $configuracao->nome) }}" required>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('tipo') is-invalid @enderror" id="tipo" name="tipo" required>
                                        ...
                                        <option value="tamanho" {{ old('tipo', $configuracao->tipo) == 'tamanho' ? 'selected' : '' }}>Tamanho</option>
                                        <option value="sabor" {{ old('tipo', $configuracao->tipo) == 'sabor' ? 'selected' : '' }}>Sabor</option>
                                        <option value="ingrediente" {{ old('tipo', $configuracao->tipo) == 'ingrediente' ? 'selected' : '' }}>Ingrediente</option>
                                        <option value="complemento" {{ old('tipo', $configuracao->tipo) == 'complemento' ? 'selected' : '' }}>Complemento</option>
                                        <option value="borda" {{ old('tipo', $configuracao->tipo) == 'borda' ? 'selected' : '' }}>Borda</option>
                                        <option value="temperatura" {{ old('tipo', $configuracao->tipo) == 'temperatura' ? 'selected' : '' }}>Temperatura</option>
                                        <option value="personalizacao" {{ old('tipo', $configuracao->tipo) == 'personalizacao' ? 'selected' : '' }}>Personalização</option>
                                        <option value="outro" {{ old('tipo', $configuracao->tipo) == 'outro' ? 'selected' : '' }}>Outro</option>
                                    </select>
                                    @error('tipo')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('produto_id') is-invalid @enderror" id="produto_id" name="produto_id">
                                ...
                                <option value="{{ $produto->id }}" {{ old('produto_id', $configuracao->produto_id) == $produto->id ? 'selected' : '' }}>
                                    {{ $produto->nome }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('descricao') is-invalid @enderror" 
                                      id="descricao" name="descricao" rows="3">{{ old('descricao', $configuracao->descricao) }}</textarea>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div id="itens-container" class="dynamic-items">
                            @foreach($configuracao->itens->sortBy('ordem') as $index => $item)
                            <div class="item-preview existing-item" data-index="{{ $index }}" data-item-id="{{ $item->id }}">
                                <input type="hidden" name="itens[{{ $index }}][id]" value="{{ $item->id }}">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div class="flex-grow-1">
                                        <input type="text" name="itens[{{ $index }}][nome]" class="form-control form-control-sm mb-2" 
                                               placeholder="Nome do item" value="{{ $item->nome }}" required onchange="updatePreview()">
                                        <div class="row">
                                            <div class="col-6">
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text">R$</span>
                                                    <input type="number" name="itens[{{ $index }}][preco_adicional]" class="form-control" 
                                                           placeholder="0,00" step="0.01" value="{{ $item->preco_adicional }}" onchange="updatePreview()">
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <input type="number" name="itens[{{ $index }}][ordem]" class="form-control form-control-sm" 
                                                       placeholder="Ordem" value="{{ $item->ordem }}" min="1" onchange="updatePreview()">
                                            </div>
                                        </div>
                                    </div>
                                    <button type="button" class="btn btn-outline-danger btn-sm ms-2" onclick="removeItem({{ $index }})">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                                <textarea name="itens[{{ $index }}][descricao]" class="form-control form-control-sm mb-2" 
                                          placeholder="Descrição (opcional)" rows="2" onchange="updatePreview()">{{ $item->descricao }}</textarea>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="itens[{{ $index }}][ativo]" 
                                           id="item_ativo_{{ $index }}" {{ $item->ativo ? 'checked' : '' }} onchange="updatePreview()">
                                    <label class="form-check-label" for="item_ativo_{{ $index }}">
                                        Item ativo
                                    </label>
                                </div>
                                <input type="hidden" name="itens[{{ $index }}][_destroy]" value="0" class="destroy-input">
                            </div>
                            @endforeach
                        </div>
                        ...
                        @else
                        <div class="text-center py-3" id="no-items-message" style="display: none;">
                            <i class="fas fa-inbox fa-2x text-muted mb-2"></i>
                            <p class="text-muted mb-0">Clique em "Adicionar Item" para criar os itens desta configuração</p>
                        </div>
                        @endif
            ...
            <div class="col-lg-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="obrigatorio" name="obrigatorio" 
                                       {{ old('obrigatorio', $configuracao->obrigatorio) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="multipla_selecao" name="multipla_selecao" 
                                       {{ old('multipla_selecao', $configuracao->multipla_selecao) ? 'checked' : '' }} onchange="toggleMaxSelecoes()">
                        ...
                        <div class="mb-3" id="max-selecoes-group" style="display: {{ old('multipla_selecao', $configuracao->multipla_selecao) ? 'block' : 'none' }};">
                            ...
                            <input type="number" class="form-control @error('max_selecoes') is-invalid @enderror" 
                                   id="max_selecoes" name="max_selecoes" value="{{ old('max_selecoes', $configuracao->max_selecoes) }}" min="1">
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('comerciantes.produtos.configuracoes.show', $configuracao) }}" class="btn btn-secondary">
...
<template id="item-template">
    <div class="item-preview new-item" data-index="__INDEX__">
        <div class="d-flex justify-content-between align-items-start mb-2">
            <div class="flex-grow-1">
...
@push('scripts')
<script>
let itemIndex = {{ $configuracao->itens->count() }};

// Definir funções globalmente
window.toggleMaxSelecoes = function() {
...
window.removeItem = function(index) {
    ...
    if (item) {
        // Se é um item existente, marcar para exclusão
        if (item.classList.contains('existing-item')) {
            const destroyInput = item.querySelector('.destroy-input');
            if (destroyInput) {
                destroyInput.value = '1';
            }
            item.style.display = 'none';
        } else {
            // Se é um item novo, remover da DOM
            item.remove();
        }
        
        updatePreview();
        
        // Verificar se deve mostrar mensagem de "sem itens"
        const visibleItems = document.querySelectorAll('#itens-container .item-preview:not([style*="display: none"])');
...
window.updatePreview = function() {
    ...
    // Adicionar itens visíveis
    const itens = document.querySelectorAll('#itens-container .item-preview:not([style*="display: none"])');
    if (itens.length > 0) {
        ...
        preview += '<p class="text-muted"><small>Nenhum item ativo</small></p>';
    }
...
document.getElementById('configuracaoForm').addEventListener('submit', function(e) {
    ...
    if (itens.length === 0) {
        e.preventDefault();
        alert('A configuração deve ter pelo menos um item ativo.');
        return false;
    }
