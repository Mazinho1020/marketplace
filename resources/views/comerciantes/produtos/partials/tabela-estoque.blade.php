@if($produtos->count() > 0)
                    ...
                    <tr>
                                ...
                                <div>
                                    ...
                                    <div class="d-flex gap-2">
                                        @if($produto->categoria)
                                            <span class="badge bg-info text-dark">{{ $produto->categoria->nome }}</span>
                                        @endif
                                        @if($produto->marca)
                        ...
                        <td class="text-center">
                            <span class="fs-5 fw-bold 
                                @if($tipo === 'zerado') text-danger
                                @elseif($tipo === 'baixo') text-warning
                                @elseif($tipo === 'critico') text-info
                                @else text-success
                                @endif">
                        ...
                        <td class="text-center">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('comerciantes.produtos.show', $produto->id) }}" 
    ...
    @if($tipo === 'normal' && $produtos->count() >= 50)
        <div class="alert alert-info mt-3">
            <i class="fas fa-info-circle me-2"></i>
            Exibindo apenas os primeiros 50 produtos com estoque normal. 
            Para ver todos, acesse a <a href="{{ route('comerciantes.produtos.index') }}">lista completa de produtos</a>.
        </div>
    @endif
@else
    <div class="text-center py-5">
        <i class="fas fa-check-circle text-success fs-1 mb-3"></i>
        <h5 class="text-muted">{{ $mensagemVazia }}</h5>
    </div>
@endif
...
$('#formAtualizarEstoque').on('submit', function(e) {
    ...
    $.ajax({
        url: `{{ url('comerciantes/produtos') }}/${produtoIdAtual}/estoque`,
        type: 'PATCH',
        data: formData,
        processData: false,
        contentType: false,
