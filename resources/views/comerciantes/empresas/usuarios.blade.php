...
@section('title', 'Usuários - ' . $empresa->nome)
            ...
            <div class="col-md-8">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        ...
                        <span class="fw-medium">{{ $empresa->proprietario?->nome ?? 'Sem proprietário' }}</span>
                    </div>
                </div>
            </div>
...
<div class="card">
    <div class="card-header">
        <h6 class="card-title mb-0">
            ...
            Usuários Vinculados ({{ $empresa->usuariosVinculados->count() }})
        </h6>
    </div>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar-circle bg-primary text-white me-3">
                                        {{ substr($vinculo->nome ?? 'U', 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="fw-medium">{{ $vinculo->nome ?? 'Nome não disponível' }}</div>
                            ...
                            <td>
                                <span class="badge bg-{{ $vinculo->pivot->perfil === 'proprietario' ? 'danger' : ($vinculo->pivot->perfil === 'administrador' ? 'warning' : 'info') }}">
                                    {{ ucfirst($vinculo->pivot->perfil ?? 'indefinido') }}
                                </span>
                            </td>
                            <td>
                                <span class="badge bg-{{ $vinculo->pivot->status === 'ativo' ? 'success' : 'secondary' }}">
                                    {{ ucfirst($vinculo->pivot->status ?? 'indefinido') }}
                                </span>
                            </td>
                            <td>
                                <small>{{ $vinculo->pivot->data_vinculo ? \Carbon\Carbon::parse($vinculo->pivot->data_vinculo)->format('d/m/Y H:i') : 'Data não disponível' }}</small>
                            </td>
