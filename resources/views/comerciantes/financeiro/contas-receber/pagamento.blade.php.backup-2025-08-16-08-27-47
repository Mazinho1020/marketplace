...
@section('title', 'Registrar Recebimento')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('comerciantes.empresas.financeiro.contas-receber.index', $empresa) }}">Contas a Receber</a></li>
        ...
        <div>
            <a href="{{ route('comerciantes.empresas.financeiro.contas-receber.index', $empresa) }}" class="btn btn-outline-secondary">
            ...
            <div class="alert alert-info">
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <span class="text-muted">{{ $contaReceber->descricao }}</span>
                    </div>
                    <div class="col-md-3">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form id="formRecebimento" method="POST" action="{{ route('comerciantes.empresas.financeiro.contas-receber.recebimentos.store', ['empresa' => $empresa->id, 'id' => $contaReceber->id ?? 0]) }}">
                @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="valor" name="valor" 
                                       step="0.01" min="0.01" max="{{ $saldoReceber ?? 9999999 }}" 
                                       value="{{ number_format($saldoReceber ?? 0, 2, '.', '') }}" required aria-describedby="valor_help">
                            </div>
                            <div id="valor_help" class="form-text">
                                @if(isset($saldoReceber))
                                    Máximo: R$ {{ number_format($saldoReceber, 2, ',', '.') }}
                                @endif
                            </div>
                        </div>
                    </div>
...
window.testarRecebimento = function() {
    ...
    const lancamentoId = {{ $lancamento->id ?? 1 }};
...
// Variáveis globais
const empresaId = {{ $empresa->id ?? 1 }};
const lancamentoId = {{ $contaReceber->id ?? 1 }};
let saldoReceber = {{ $saldoReceber ?? 0 }};
    ...
    document.getElementById('formRecebimento').addEventListener('submit', function(e) {
        ...
        .then(({ status, data }) => {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    window.location.href = '{{ route("comerciantes.empresas.financeiro.contas-receber.index", $empresa) }}';
                }, 2000);
