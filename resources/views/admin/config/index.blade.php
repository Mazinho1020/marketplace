                            ...
                            <select name="group" id="group" class="form-select">
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->codigo }}" 
                                        {{ ($groupFilter ?? '') === ($group->codigo ?? '') ? 'selected' : '' }}>
                                        {{ $group->nome }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="site" id="site" class="form-select">
                                ...
                                @foreach($sites as $site)
                                    <option value="{{ $site->codigo }}" 
                                        {{ ($siteFilter ?? '') === ($site->codigo ?? '') ? 'selected' : '' }}>
                                        {{ $site->nome }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="environment" id="environment" class="form-select">
                                ...
                                @foreach($environments as $environment)
                                    <option value="{{ $environment->codigo }}" 
                                        {{ ($environmentFilter ?? '') === ($environment->codigo ?? '') ? 'selected' : '' }}>
                                        {{ $environment->nome }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mt-3">
                        <div class="col-12">
                            <div class="d-flex gap-2 flex-wrap">
                                ...
                                <a href="{{ route('admin.config.export') }}" 
                                    class="btn btn-info">
                                        ...
                                        @foreach($groups as $group)
                                            <li>
                                                <a class="dropdown-item" 
                                                    href="{{ route('admin.config.group', $group->codigo) }}">
                                                    <i class="{{ $group->icone_class }} me-2"></i>
                                                    {{ $group->nome }}
                                                </a>
                                            </li>
                                        @endforeach
                        ...
                        @foreach($configsByGroup as $groupName => $groupConfigs)
                            <div class="config-group mb-4">
                                <h5 class="text-primary border-bottom pb-2 mb-3">
                                    <i class="{{ $groupConfigs->first()->group->icone_class ?? 'uil uil-folder' }} me-2"></i>
                                    {{ $groupName }}
                                </h5>
                                            ...
                                            <div class="card border h-100">
                                                <div class="card-body">
                                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                <li>
                                                                    <a class="dropdown-item" 
                                                                        href="{{ route('admin.config.edit', $config->id) }}">
                                                                ...
                                                                <li>
                                                                    <a class="dropdown-item" 
                                                                        href="{{ route('admin.config.history', $config->id) }}">
                                                    ...
                                                    <div class="mb-2">
                                                        ...
                                                        <span class="badge bg-secondary">{{ ucfirst($config->tipo) }}</span>
                                                    </div>

                                                    <!-- Formulário de edição rápida -->
                                                    <form class="config-form" data-config-id="{{ $config->id }}">
                                                        <div class="mb-2">
                                                        ...
                                                        <div class="row g-2 mb-2">
                                                            ...
                                                            <div class="col">
                                                                <select class="form-select form-select-sm" name="ambiente_id">
                                                                    <option value="">Todos os ambientes</option>
                                                                    @foreach($environments as $environment)
                                                                        <option value="{{ $environment->id }}">{{ $environment->nome }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        </div>
...
function deleteConfig(configId) {
    ...
    deleteForm.action = `{{ route('admin.config.index') }}/${configId}`;
