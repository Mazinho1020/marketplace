    ...
    <script>
        ...
        document.querySelectorAll('.config-form').forEach(form => {
            form.addEventListener('submit', async function(e) {
                ...
                if (!configChave) {
                    alert('Erro: Chave da configuração não encontrada');
                    return;
                }
                ...
                try {
                    const response = await fetch('/admin/config/set-value', {
                        method: 'POST',
                        headers: {
        ...
        async function clearCache() {
            try {
                const response = await fetch('/admin/config/clear-cache', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json'
                ...
                if (result.success) {
                    showSuccess(result.message || 'Cache limpo com sucesso!');
                } else {
                    showError(result.message || 'Erro ao limpar cache');
                }
            } catch (error) {
