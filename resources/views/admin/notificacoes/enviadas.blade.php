...
@section('title', 'Notificações Enviadas')
...
function carregarNotificacoes() {
    ...
    fetch(`{{ route('admin.notificacoes.enviadas.dados') }}?${params}`)
...
function carregarEstatisticas() {
    ...
    fetch(`{{ route('admin.notificacoes.enviadas.estatisticas') }}?${params}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function verDetalhes(notifId) {
    ...
    fetch(`{{ url('admin/notificacoes/enviadas') }}/${notifId}/detalhes`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function carregarLogs(notifId) {
    fetch(`{{ url('admin/notificacoes/enviadas') }}/${notifId}/logs`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function reenviarNotificacao(notifId) {
    ...
    if (confirm('Deseja realmente reenviar esta notificação?')) {
        ...
        fetch(`{{ url('admin/notificacoes/enviadas') }}/${notifId}/reenviar`, {
            method: 'POST',
            headers: {
...
function verLogs(notifId) {
    window.open(`{{ url('admin/notificacoes/enviadas') }}/${notifId}/logs`, '_blank');
}
...
function exportarDados() {
    ...
    window.open(`{{ route('admin.notificacoes.enviadas.exportar') }}?${params}`, '_blank');
}
