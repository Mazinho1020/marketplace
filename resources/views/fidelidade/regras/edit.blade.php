...
@section('title', 'Editar Regra de Cashback')
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1">
                        ...
                        Editar Regra #{{ $regra->id }}
                    </h1>
                    <p class="text-muted mb-0">Modifique as configurações da regra de cashback</p>
                </div>
                <div>
                    <a href="{{ route('fidelidade.regras.show', $regra->id) }}" class="btn btn-outline-info">
        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Editar Configuração da Regra</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('fidelidade.regras.update', $regra->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <select name="tipo_regra" class="form-select @error('tipo_regra') is-invalid @enderror"
                                    ...
                                    <option value="categoria" {{ old('tipo_regra', $regra->tipo_regra) === 'categoria' ?
                                        'selected' : '' }}>Por Categoria</option>
                                    <option value="produto" {{ old('tipo_regra', $regra->tipo_regra) === 'produto' ?
                                        'selected' : '' }}>Por Produto</option>
                                    <option value="dia_semana" {{ old('tipo_regra', $regra->tipo_regra) === 'dia_semana'
                                        ? 'selected' : '' }}>Por Dia da Semana</option>
                                    <option value="horario" {{ old('tipo_regra', $regra->tipo_regra) === 'horario' ?
                                        'selected' : '' }}>Por Horário</option>
                                    <option value="primeira_compra" {{ old('tipo_regra', $regra->tipo_regra) ===
                                        'primeira_compra' ? 'selected' : '' }}>Primeira Compra</option>
                                </select>
                                @error('tipo_regra')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="percentual_cashback"
                                    ...
                                    min="0" max="100"
                                    value="{{ old('percentual_cashback', $regra->percentual_cashback) }}" required>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="valor_maximo_cashback"
                                        ...
                                        step="0.01" min="0"
                                        value="{{ old('valor_maximo_cashback', $regra->valor_maximo_cashback) }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="empresa_id"
                                    ...
                                    value="{{ old('empresa_id', $regra->empresa_id) }}">
                                @error('empresa_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-4" id="condicoes-especificas">
                            <!-- Dia da Semana -->
                            <div class="col-md-6" id="campo-dia-semana">
                                <label class="form-label">Dia da Semana</label>
                                <select name="dia_semana" class="form-select">
                                    <option value="">Selecione o dia</option>
                                    <option value="0" {{ old('dia_semana', $regra->dia_semana) == '0' ? 'selected' : ''
                                        }}>Domingo</option>
                                    <option value="1" {{ old('dia_semana', $regra->dia_semana) == '1' ? 'selected' : ''
                                        }}>Segunda-feira</option>
                                    <option value="2" {{ old('dia_semana', $regra->dia_semana) == '2' ? 'selected' : ''
                                        }}>Terça-feira</option>
                                    <option value="3" {{ old('dia_semana', $regra->dia_semana) == '3' ? 'selected' : ''
                                        }}>Quarta-feira</option>
                                    <option value="4" {{ old('dia_semana', $regra->dia_semana) == '4' ? 'selected' : ''
                                        }}>Quinta-feira</option>
                                    <option value="5" {{ old('dia_semana', $regra->dia_semana) == '5' ? 'selected' : ''
                                        }}>Sexta-feira</option>
                                    <option value="6" {{ old('dia_semana', $regra->dia_semana) == '6' ? 'selected' : ''
                                        }}>Sábado</option>
                                </select>
                            </div>

                            <!-- Horários -->
                            <div class="col-md-3" id="campo-horario-inicio">
                                <label class="form-label">Horário Início</label>
                                <input type="time" name="horario_inicio" class="form-control"
                                    value="{{ old('horario_inicio', $regra->horario_inicio ? substr($regra->horario_inicio, 0, 5) : '') }}">
                            </div>

                            <div class="col-md-3" id="campo-horario-fim">
                                <label class="form-label">Horário Fim</label>
                                <input type="time" name="horario_fim" class="form-control"
                                    value="{{ old('horario_fim', $regra->horario_fim ? substr($regra->horario_fim, 0, 5) : '') }}">
                            </div>

                            <!-- Referência ID (para categoria/produto) -->
                            <div class="col-md-6" id="campo-referencia">
                                <label class="form-label">ID de Referência</label>
                                <input type="number" name="referencia_id" class="form-control"
                                    value="{{ old('referencia_id', $regra->referencia_id) }}">
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <div class="alert alert-info">
                                    <h6><i class="fas fa-info-circle me-2"></i>Informações da Regra</h6>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <strong>Criada em:</strong> {{
                                            \Carbon\Carbon::parse($regra->created_at)->format('d/m/Y H:i') }}
                                        </div>
                                        <div class="col-md-6">
                                            <strong>Última atualização:</strong> {{
                                            \Carbon\Carbon::parse($regra->updated_at)->format('d/m/Y H:i') }}
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    toggleCampos(); // Executar na carga da página para mostrar campos corretos
});
</script>
@endsection