<?php $__env->startSection('title', 'Produtos Relacionados'); ?>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">
                        <strong><?php echo e($produto->nome); ?></strong> (<?php echo e($produto->sku); ?>)
                    </p>
                </div>
                <div>
                    ...
                    <a href="<?php echo e(route('comerciantes.produtos.index')); ?>" class="btn btn-secondary">
                        ← Voltar aos Produtos
                    </a>
                </div>
            </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <select name="tipo_relacao" id="tipo_relacao" class="form-control">
                                    ...
                                    <?php $__currentLoopData = \App\Models\ProdutoRelacionado::getTiposRelacao(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($key); ?>" <?php echo e(request('tipo_relacao') == $key ? 'selected' : ''); ?>>
                                        <?php echo e($label); ?>
                            ...
                            <div class="col-md-5">
                                ...
                                <div class="d-flex">
                                    ...
                                    <a href="<?php echo e(route('comerciantes.produtos.relacionados.index', $produto->id)); ?>" class="btn btn-outline-secondary">Limpar</a>
                                </div>
                            </div>
                        </div>
                    </form>
            ...
            <?php $__currentLoopData = $relacionados->groupBy('tipo_relacao'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tipo => $grupo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        ...
                        <?php endswitch; ?>
                        <span class="badge badge-primary"><?php echo e($grupo->count()); ?></span>
                    </h5>
                    <small class="text-muted">Arraste para reordenar</small>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-striped mb-0 sortable-table" data-tipo="<?php echo e($tipo); ?>">
                            <thead>
                                <tr>
                                    <th width="50">📋</th>
                                    <th>Produto</th>
                            ...
                            <tbody class="sortable-body">
                                <?php $__currentLoopData = $grupo->sortBy('ordem'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relacionado): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr data-id="<?php echo e($relacionado->id); ?>" data-ordem="<?php echo e($relacionado->ordem); ?>">
                                    <td class="drag-handle" style="cursor: move;">
                                        <i class="fas fa-grip-vertical text-muted"></i>
                                    </td>
                                    <td>
                                        <strong><?php echo e($relacionado->produtoRelacionado->nome); ?></strong>
                                    </td>
                                    <td>
                                        <code><?php echo e($relacionado->produtoRelacionado->sku); ?></code>
                                    </td>
                                    <td>
                                        <span class="text-success font-weight-bold">
                                            R$ <?php echo e(number_format($relacionado->produtoRelacionado->preco_venda, 2, ',', '.')); ?>

                                        </span>
                                    </td>
                                    <td>
                                        <?php if($relacionado->produtoRelacionado->controla_estoque): ?>
                                        <span class="badge <?php echo e($relacionado->produtoRelacionado->estoque_atual > 0 ? 'badge-success' : 'badge-danger'); ?>">
                                            <?php echo e($relacionado->produtoRelacionado->estoque_atual); ?>
                                    ...
                                    <td>
                                        <input type="number" class="form-control ordem-input"
                                            value="<?php echo e($relacionado->ordem); ?>"
                                            data-id="<?php echo e($relacionado->id); ?>"
                                            style="width: 80px;" min="0">
                                    </td>
                                    <td>
                                        <?php echo $relacionado->status_badge; ?>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <button type="button" class="btn btn-outline-primary btn-sm btn-editar"
                                                data-id="<?php echo e($relacionado->id); ?>"
                                                data-tipo="<?php echo e($relacionado->tipo_relacao); ?>"
                                                data-ordem="<?php echo e($relacionado->ordem); ?>"
                                                data-ativo="<?php echo e($relacionado->ativo); ?>"
                                                data-bs-toggle="tooltip" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form method="POST"
                                                action="<?php echo e(route('comerciantes.produtos.relacionados.destroy', [$produto->id, $relacionado->id])); ?>"
                                                class="d-inline">
                                                <?php echo csrf_field(); ?>
...
<div class="modal fade" id="modalNovoRelacionado" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" action="<?php echo e(route('comerciantes.produtos.relacionados.store', $produto->id)); ?>">
                <?php echo csrf_field(); ?>
...
<script>
    ...
    function loadProductsForSelect() {
        const elemento = $('#modal_produto_relacionado_id');
        const buscarUrl = '<?php echo e(route("comerciantes.produtos.relacionados.buscar", $produto->id)); ?>';
                ...
                ajax: {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        if (xhr.responseText && xhr.responseText.indexOf('<!DOCTYPE') === 0) {
                            ...
                            setTimeout(() => {
                                window.location.href = '<?php echo e(route("comerciantes.login")); ?>';
                            }, 2000);
                        }
                    },
                    cache: true
    ...
    function initializeApp() {
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $('#formEditarRelacionado').attr('action',
                '<?php echo e(route("comerciantes.produtos.relacionados.update", [$produto->id, ":id"])); ?>'.replace(':id', id)
            );
        ...
        function updateOrdem(tipo) {
            ...
            $.ajax({
                url: '<?php echo e(route("comerciantes.produtos.relacionados.update-ordem", $produto->id)); ?>',
                method: 'POST',
                data: {
        ...
        function updateOrdemIndividual(id, ordem) {
            $.ajax({
                url: '<?php echo e(route("comerciantes.produtos.relacionados.update", [$produto->id, ":id"])); ?>'.replace(':id', id),
                method: 'PUT',
                data: {
                    _token: '<?php echo e(csrf_token()); ?>',
                    ordem: ordem,
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('comerciantes.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\marketplace\resources\views/comerciantes/produtos/relacionados/index.blade.php ENDPATH**/ ?>