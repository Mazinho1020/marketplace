<?php $__env->startSection('title', 'Detalhes do Webhook'); ?>
        ...
        <div class="row align-items-center">
            <div class="col">
                <h4 class="page-title mb-0">
                    ...
                    Detalhes do Webhook #<?php echo e($webhook->id); ?>
                ...
                <nav aria-label="breadcrumb" class="mt-2">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="<?php echo e(route('admin.payments.webhooks')); ?>">Webhooks</a></li>
                        <li class="breadcrumb-item active">Detalhes</li>
                    </ol>
                </nav>
            </div>
            <div class="col-auto">
                <a href="<?php echo e(route('admin.payments.webhooks')); ?>" class="btn btn-outline-secondary">
        ...
        <div class="col-lg-8">
                        ...
                        <div class="col-md-6">
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td><?php echo e($webhook->created_at->format('d/m/Y H:i:s')); ?></td>
                                </tr>
                                <tr>
                                    <td><strong>Processado em:</strong></td>
                                    <td>
                                        <?php if($webhook->processed_at): ?>
                                            <?php echo e($webhook->processed_at->format('d/m/Y H:i:s')); ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-danger"><?php echo e($webhook->error_message); ?></span>
                                    </td>
                                </tr>
                                <?php endif; ?>
                                <?php if($webhook->gateway_signature): ?>
                                <tr>
                                    <td><strong>Assinatura:</strong></td>
                                    <td><code class="small"><?php echo e(Str::limit($webhook->gateway_signature, 30)); ?></code></td>
                                </tr>
                                <?php endif; ?>
                            </table>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <a href="<?php echo e(route('admin.payments.transaction-details', $webhook->transaction->id)); ?>" class="text-decoration-none">
                                    <code>#<?php echo e($webhook->transaction->external_id ?? $webhook->transaction->id); ?></code>
                                </a>
                            </div>
                            <div class="mb-3">
                                <strong>Valor:</strong>
                                <span class="text-success">R$ <?php echo e(number_format($webhook->transaction->amount, 2, ',', '.')); ?></span>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <strong>Status:</strong>
                                <?php if($webhook->transaction->status === 'approved'): ?>
                                    <span class="badge bg-success">Aprovada</span>
                                <?php elseif($webhook->transaction->status === 'pending'): ?>
                                    <span class="badge bg-warning">Pendente</span>
                                <?php elseif($webhook->transaction->status === 'rejected'): ?>
                                    <span class="badge bg-danger">Rejeitada</span>
                                <?php else: ?>
                                    <span class="badge bg-secondary"><?php echo e(ucfirst($webhook->transaction->status)); ?></span>
                                <?php endif; ?>
                            </div>
                            <div class="mb-3">
                                <strong>Gateway:</strong>
                                <?php echo e($webhook->transaction->gateway->name ?? 'N/A'); ?>

                            </div>
                        </div>
                    </div>
                    <a href="<?php echo e(route('admin.payments.transaction-details', $webhook->transaction->id)); ?>" class="btn btn-outline-primary btn-sm">
            ...
            <div class="card">
                ...
                <div class="card-body" id="rawDataContainer" style="display: none;">
                    <pre class="bg-light p-3 rounded" style="max-height: 400px; overflow-y: auto;"><code><?php echo e(json_encode($webhook->gateway_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)); ?></code></pre>
                </div>
            </div>
            <?php endif; ?>
        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        ...
                        <?php if($webhook->transaction): ?>
                        <a href="<?php echo e(route('admin.payments.transaction-details', $webhook->transaction->id)); ?>" class="btn btn-outline-primary">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div><code><?php echo e($webhook->ip_address); ?></code></div>
                    </div>
                    <?php endif; ?>
                    
                    <?php if($webhook->user_agent): ?>
                    <div class="mb-3">
                        <small class="text-muted">User Agent:</small>
                        <div class="small"><?php echo e(Str::limit($webhook->user_agent, 50)); ?></div>
                    </div>
                    <?php endif; ?>
                    
                    <?php if($webhook->headers): ?>
                    <div class="mb-3">
                        ...
                        <div id="headersContainer" style="display: none;" class="mt-2">
                            <pre class="bg-light p-2 rounded small" style="max-height: 200px; overflow-y: auto;"><code><?php echo e(json_encode($webhook->headers, JSON_PRETTY_PRINT)); ?></code></pre>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="mb-3">
                        ...
                        <div><?php echo e($webhook->gateway_data ? Str::of(json_encode($webhook->gateway_data))->length() . ' bytes' : 'N/A'); ?></div>
                    </div>
                </div>
            </div>
        </div>
...
function reprocessWebhook() {
    if (confirm('Tem certeza que deseja reprocessar este webhook?')) {
        fetch(`/admin/payments/webhooks/<?php echo e($webhook->id); ?>/reprocess`, {
            method: 'POST',
            headers: {
...
function exportWebhook() {
    const data = <?php echo json_encode($webhook, 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\marketplace\resources\views/admin/payments/webhook-details.blade.php ENDPATH**/ ?>