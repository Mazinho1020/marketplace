<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        // Tabela de grupos de configuração
        Schema::create('config_groups', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->string('codigo', 50)->comment('Código único do grupo');
            $table->string('nome', 100)->comment('Nome de exibição do grupo');
            $table->text('descricao')->nullable()->comment('Descrição do grupo');
            $table->unsignedBigInteger('grupo_pai_id')->nullable()->comment('ID do grupo pai');
            $table->string('icone', 50)->nullable()->comment('Classe de ícone para interface');
            $table->string('icone_class', 50)->nullable()->comment('Classe de ícone para interface');
            $table->integer('ordem')->default(0)->comment('Ordem de exibição do grupo');
            $table->boolean('ativo')->default(true)->comment('Status do grupo');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'codigo']);
            $table->index(['empresa_id']);
            $table->index(['grupo_pai_id']);
        });

        // Tabela de ambientes de configuração
        Schema::create('config_environments', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->string('codigo', 50)->comment('Código único do ambiente');
            $table->string('nome', 100)->comment('Nome de exibição do ambiente');
            $table->text('descricao')->nullable()->comment('Descrição do ambiente');
            $table->boolean('is_producao')->default(false)->comment('Ambiente de produção');
            $table->boolean('ativo')->default(true)->comment('Status do ambiente');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'codigo']);
            $table->index(['empresa_id']);
        });

        // Tabela de sites de configuração
        Schema::create('config_sites', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->string('codigo', 50)->comment('Código único do site');
            $table->string('nome', 100)->comment('Nome de exibição do site');
            $table->text('descricao')->nullable()->comment('Descrição do site');
            $table->string('base_url_padrao')->nullable()->comment('URL base padrão do site');
            $table->boolean('ativo')->default(true)->comment('Status do site');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'codigo']);
            $table->index(['empresa_id']);
        });

        // Tabela de definições de configuração
        Schema::create('config_definitions', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->string('chave', 100)->comment('Nome da chave de configuração');
            $table->string('nome', 100)->comment('Nome amigável da configuração');
            $table->text('descricao')->nullable()->comment('Descrição da configuração');
            $table->enum('tipo', ['string', 'integer', 'float', 'boolean', 'array', 'json', 'url', 'email', 'password'])->default('string')->comment('Tipo de dado');
            $table->unsignedBigInteger('grupo_id')->nullable()->comment('Grupo ao qual pertence');
            $table->text('valor_padrao')->nullable()->comment('Valor padrão');
            $table->boolean('obrigatorio')->default(false)->comment('Se é obrigatória');
            $table->integer('min_length')->nullable()->comment('Tamanho mínimo');
            $table->integer('max_length')->nullable()->comment('Tamanho máximo');
            $table->string('regex_validacao')->nullable()->comment('Regex para validação');
            $table->text('opcoes')->nullable()->comment('Opções possíveis (JSON)');
            $table->boolean('editavel')->default(true)->comment('Se pode ser editado via interface');
            $table->boolean('avancado')->default(false)->comment('Se é uma configuração avançada');
            $table->integer('ordem')->default(0)->comment('Ordem de exibição');
            $table->text('dica')->nullable()->comment('Dica de ajuda na interface');
            $table->text('ajuda')->nullable()->comment('Texto de ajuda detalhado');
            $table->boolean('ativo')->default(true)->comment('Status da definição');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'chave']);
            $table->index(['empresa_id']);
            $table->index(['grupo_id']);
        });

        // Tabela de valores de configuração
        Schema::create('config_values', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->unsignedBigInteger('config_id')->comment('ID da definição da configuração');
            $table->unsignedBigInteger('site_id')->nullable()->comment('ID do site específico');
            $table->unsignedBigInteger('ambiente_id')->nullable()->comment('ID do ambiente específico');
            $table->text('valor')->nullable()->comment('Valor da configuração');
            $table->unsignedInteger('usuario_id')->nullable()->comment('ID do usuário que fez a alteração');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'config_id', 'site_id', 'ambiente_id'], 'unique_config_value');
            $table->index(['empresa_id']);
            $table->index(['config_id']);
            $table->index(['site_id']);
            $table->index(['ambiente_id']);

            $table->foreign('config_id')->references('id')->on('config_definitions');
            $table->foreign('site_id')->references('id')->on('config_sites');
            $table->foreign('ambiente_id')->references('id')->on('config_environments');
        });

        // Tabela de histórico de configuração
        Schema::create('config_history', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->unsignedBigInteger('config_id')->comment('ID da configuração');
            $table->unsignedBigInteger('site_id')->nullable()->comment('ID do site');
            $table->unsignedBigInteger('ambiente_id')->nullable()->comment('ID do ambiente');
            $table->enum('acao', ['create', 'update', 'delete'])->comment('Ação realizada');
            $table->text('valor_anterior')->nullable()->comment('Valor anterior');
            $table->text('valor_novo')->nullable()->comment('Novo valor');
            $table->unsignedInteger('usuario_id')->nullable()->comment('ID do usuário');
            $table->string('usuario_nome', 100)->nullable()->comment('Nome do usuário');
            $table->string('ip', 45)->nullable()->comment('IP do usuário');
            $table->text('user_agent')->nullable()->comment('User-Agent do navegador');
            $table->text('contexto_info')->nullable()->comment('Informações de contexto');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->index(['empresa_id']);
            $table->index(['config_id']);
            $table->index(['usuario_id']);

            $table->foreign('config_id')->references('id')->on('config_definitions');
        });

        // Tabela de conexões de banco de dados
        Schema::create('config_db_connections', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->string('nome', 100)->comment('Nome descritivo da conexão');
            $table->unsignedBigInteger('ambiente_id')->comment('ID do ambiente');
            $table->enum('driver', ['mysql', 'postgresql', 'sqlite', 'sqlserver'])->default('mysql')->comment('Driver do banco de dados');
            $table->string('host')->comment('Host do servidor');
            $table->integer('porta')->default(3306)->comment('Porta do servidor');
            $table->string('banco', 100)->comment('Nome do banco de dados');
            $table->string('usuario', 100)->comment('Usuário do banco');
            $table->text('senha')->comment('Senha criptografada');
            $table->string('charset', 20)->default('utf8mb4')->comment('Charset do banco');
            $table->string('collation', 100)->default('utf8mb4_unicode_ci')->comment('Collation do banco');
            $table->string('prefixo', 50)->nullable()->comment('Prefixo das tabelas');
            $table->boolean('padrao')->default(false)->comment('Se é a conexão padrão');
            $table->boolean('apenas_offline')->default(true)->comment('Se deve ser usada apenas offline');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->index(['empresa_id', 'ambiente_id']);
            $table->index(['ambiente_id']);

            $table->foreign('ambiente_id')->references('id')->on('config_environments');
        });

        // Tabela de mapeamento de URLs
        Schema::create('config_url_mappings', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('empresa_id')->comment('ID da empresa');
            $table->unsignedBigInteger('site_id')->comment('ID do site');
            $table->unsignedBigInteger('ambiente_id')->comment('ID do ambiente');
            $table->string('dominio')->comment('Domínio do site');
            $table->string('base_url')->comment('URL base');
            $table->string('api_url')->nullable()->comment('URL da API');
            $table->string('asset_url')->nullable()->comment('URL dos assets');
            $table->boolean('apenas_offline')->default(true)->comment('Se deve ser usado apenas offline');
            $table->string('sync_hash', 64)->nullable()->comment('Hash para sincronização');
            $table->enum('sync_status', ['pending', 'synced', 'error'])->default('pending')->comment('Status da sincronização');
            $table->timestamp('sync_data')->nullable()->comment('Data da última sincronização');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['empresa_id', 'site_id', 'ambiente_id'], 'unique_url_mapping');
            $table->index(['site_id']);
            $table->index(['ambiente_id']);

            $table->foreign('site_id')->references('id')->on('config_sites');
            $table->foreign('ambiente_id')->references('id')->on('config_environments');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('config_url_mappings');
        Schema::dropIfExists('config_db_connections');
        Schema::dropIfExists('config_history');
        Schema::dropIfExists('config_values');
        Schema::dropIfExists('config_definitions');
        Schema::dropIfExists('config_sites');
        Schema::dropIfExists('config_environments');
        Schema::dropIfExists('config_groups');
    }
};
